// Segment Tree Lazy (blaz)
// invocar con segment_tree<type, merge> ST(v);. La función merge se debe definir antes.
// ST.update(l, r, val): aplica una operación sobre el rango [l, r] (default: sumar val, modificable)
// ST.query(l, r): devuelve la op. definida en merge sobre el rango [l, r].
...
struct segment_tree{
    ...
    segment_tree(vector <T> &A){ // al invocar el segment tree con st(v) se crea el vector solo
        N = A.size();
        tree.resize(4*N);
        lazy.assign(4*N, T()); // modify default value
        build(0, 0, N-1, A);
    ...
    void update(int n, int i, int j, int l, int r, T val){
        if(l <= i && j <= r){
            lazy[n] += val; // indica la operación que se aplica al rango (modificable)
            push(n, i, j);
            return;
        }
        push(n, i, j);
